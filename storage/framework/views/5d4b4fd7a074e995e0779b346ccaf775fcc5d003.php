    ...
    <script>
        ...
        function dealAdd(d){
            if(d.error){
                ...
                $('.popup_open p:first').html('<?php echo app('translator')->getFromJson("messages.trand_on_instrument"); ?> '+d.instrument.title+' <?php echo app('translator')->getFromJson("messages.is_opened"); ?>');
                $('.popup_open p:eq(1)').html('<?php echo app('translator')->getFromJson("messages.commission_is"); ?> '+currency.value(d.fee,'USD') + ' ('+parseFloat(d.instrument.commission)*100+'%)');
            }
        }
        _onload.push(
            function(){
