<aside class="left">
    <ul>
        <li>
            ...
            <div class="item item2 flex flex-top loader" data-action="/json/instrument?limit=12" data-name="instruments-popular" data-autostart="true" data-refresh="3000" data-function="userInstruments"></div>
        </li>
        <li>
            <a href="#">@lang('messages.Instruments')</a>

            <div class="item item2 flex flex-top loader" data-action="/json/instrument" data-name="instruments" data-autostart="true" data-refresh="8000" data-function="userInstruments"></div>
    ...
    <script>
        ...
        function userInstruments(container,d,x,s){
            container.html('');
            var firstInstrument = true, informer = '',count=100;
            for(var i in d.data){
                var row=d.data[i], elementId = 'instrument_chart_id_'+row.id, diff= 100*(parseFloat(row.histo.open)-parseFloat(row.histo.close))/parseFloat(row.histo.open);
                ...
                s+='<div class="box">'
                        +'<span><img src="'+((row.from!=undefined)?row.from.image:'')+'" alt=""><img src="'+((row.to!=undefined)?row.to.image:'')+'" alt=""></span>'
                        +'<p class="viz"><i class="'+((row.histo.volation>0)?"up":"down")+'"></i>('+currency.value(diff,'')+'%)</p><p class="hidden slice">'+row.title+'</p>'
                    +'</div>';
                s+='<div class="box">'
        ...
        function chooseInstrument(id){
            ...
            $("#charttitle").html('<p>'+window.instrument.title+'</p>&nbsp;&nbsp<p>'+window.instrument.histo.open+'</p>');
            $("#chartimage").html('<div class"box"><span><img src="'+window.instrument.from.image+'" /><img src="'+window.instrument.to.image+'" /></span><p class="hidden slice">'+window.instrument.title+'</p></div>');
